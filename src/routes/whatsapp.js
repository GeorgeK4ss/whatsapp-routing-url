...
router.get('/wa', asyncHandler(async (req, res) => {
  const userAgent = req.headers["user-agent"] || "";
  const ip = geoService.getClientIP(req);
  
  logger.info('WhatsApp redirect request', {
    ip,
  ...
  try {
    ...
    // Get appropriate number and text
    const number = isTurkey ? config.number_tr : config.number_default;
    const prefillText = isTurkey ? config.text_tr : config.text_default;
    ...
    const whatsappUrl = `https://api.whatsapp.com/send?phone=${encodeURIComponent(number)}${textParam}`;
    
    logger.info('WhatsApp redirect generated', {
      ip,
      country,
      routing,
      number,
    ...
    logger.error('WhatsApp redirect error', {
      ip,
      error: error.message,
      stack: error.stack
    });
    ...
    const config = await configStorage.getConfig();
    const fallbackUrl = `https://api.whatsapp.com/send?phone=${encodeURIComponent(config.number_default)}`;
...
<head>
  ...
  <meta property="og:description" content="Get in touch with our team via WhatsApp"/>
  <meta property="og:type" content="website"/>
  <meta name="twitter:card" content="summary"/>
  <meta name="twitter:title" content="Contact Us"/>
  <meta name="twitter:description" content="Get in touch with our team via WhatsApp"/>
  <style>
...
<body>
  <div class="card">
    ...
    <h1>Contact Us</h1>
    <p>Get in touch with our team via WhatsApp. We're here to help!</p>
